...
class SetPasswordModal extends Component {
  ...
  getValidators() {
    if (!this.state.password) {
      return [
        validators.required(),
        value => ({ isValid: value && value.length >= 8, error: 'Password must be at least 8 characters long' }),
        value => ({ isValid: value && value.search(/[a-z]/i) >= 0, error: 'Password must contain at least one letter' }),
        value => ({ isValid: value && value.search(/[0-9]/) >= 0, error: 'Password must contain at least one digit' }),
        value => ({ isValid: value && value.search(/[!@#$%^&*]/) >= 0, error: 'Password must contain at least one special character' }),
      ];
    }
    return [
      value => ({ isValid: value === this.state.password }),
    ];
  }
        ...
        <View style={styles.container}>
          ...
          <DagForm>
            ...
            <View style={styles.buttonsContainer}>
              <DagButton
                style={StyleSheet.flatten([styles.button, styles.cancelButton, container.m15r])}
                textStyle={styles.cancelButtonText}
                text="CANCEL"
                onClick={this.onCancelClick.bind(this)}
              />
              <DagButton
                style={StyleSheet.flatten([styles.button, container.m15l])}
                text="SET"
                type="submit"
